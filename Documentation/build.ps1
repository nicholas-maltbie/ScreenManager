...
Copy-Item -Force "$project_dir\Packages\com.nickmaltbie.screenmanager\CHANGELOG.md" "$dir\changelog\CHANGELOG.md"
...
foreach ($tag in $versions)
{
    ...
    if (Test-Path "$project_dir\Packages\com.nickmaltbie.screenmanager\CHANGELOG.md")
    {
        Copy-Item -Force "$project_dir\Packages\com.nickmaltbie.screenmanager\CHANGELOG.md" "$dir\changelog\CHANGELOG.md" > $null
    }
